...
def get_data_loader(dataset_location, batch_size):
    trainvalid = torchvision.datasets.SVHN(
        dataset_location, split="train", download=True, transform=image_transform
    )

    trainset_size = int(len(trainvalid) * 0.9)
    trainset, validset = dataset.random_split(
        trainvalid, [trainset_size, len(trainvalid) - trainset_size]
    )
    ...
    testloader = torch.utils.data.DataLoader(
        torchvision.datasets.SVHN(
            dataset_location, split="test", download=True, transform=image_transform
        ),
...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.mlp = nn.Sequential(nn.ELU(), nn.Dropout(0.5), nn.Linear(512, 10))
